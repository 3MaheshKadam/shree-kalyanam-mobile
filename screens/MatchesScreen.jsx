...
const MatchesScreen = () => {
  ...
  const calculateCompatibility = (userProfile, matchProfile) => {
    ...
    fields.forEach(({ expectation, matchField }) => {
      if (userProfile[expectation] === matchProfile[matchField]) {
        matched += percentPerField;
      }
    });

    return Math.min(100, Math.round(matched));
  };
  ...
  const fetchUsers = async (pageNum = 1, isRefresh = false) => {
    ...
    try {
      ...
      if (data.success) {
        const enriched = data.data
          .filter((m) => m._id !== currentUserData.id && m.gender !== currentUserData.gender)
          .map((m) => ({
            ...m,
            age: calculateAge(m.dob),
            profilePhoto: m.profilePhoto || `https://ui-avatars.com/api/?name=${encodeURIComponent(m.name || "User")}&size=400`,
            isBlurred: !hasSubscription,
            interestSent: sentReceiverIds.includes(m._id),
            compatibility: calculateCompatibility(currentUserData, { ...m, age: calculateAge(m.dob) }),
            isNew: Math.random() > 0.7,
            lastActive: ["Recently", "Today", "1 day ago"][Math.floor(Math.random() * 3)],
          }));
        setMatches((prev) => (isRefresh || pageNum === 1 ? enriched : [...prev, ...enriched]));
  ...
  const handleSendInterest = async (receiverId) => {
    ...
    try {
      ...
      if (res.ok) {
        setMatches(matches.map((m) => (m._id === receiverId ? { ...m, interestSent: true } : m)));
        Toast.show({ type: "success", text1: "Interest sent!" });
      }
    } catch (error) {
  ...
  const tabs = [
    ...
    { id: "premium", label: "Premium", count: matches.filter((m) => m.compatibility >= 70).length },
    { id: "new", label: "New", count: matches.filter((m) => m.isNew).length },
  ];

  const filteredMatches = matches.filter((match) => {
    if (activeTab === "premium" && match.compatibility < 70) return false;
    if (activeTab === "new" && !match.isNew) return false;
    if (searchQuery && !match.currentCity?.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
  ...
  const renderHeader = () => (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 26, fontWeight: '700', color: COLORS.background, marginBottom: 4 }}>
            Discover
          </Text>
    ...
    >
      ...
      >
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
          <View style={{
            backgroundColor: item.compatibility >= 70 ? COLORS.success : COLORS.primary,
            paddingHorizontal: 10,
            paddingVertical: 4,
            borderRadius: 8,
          }}>
          ...
          {item.isNew && (
            <View style={{
              ...
              <Text style={{ color: COLORS.background, fontSize: 10, fontWeight: '600' }}>NEW</Text>
            </View>
          )}
        </View>
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            borderRadius: 10,
            alignItems: 'center',
            opacity: item.interestSent ? 0.7 : 1,
          }}
        >
          <Text style={{
            color: COLORS.background,
          ...
          <AnimatedFlatList
            ...
            ListEmptyComponent={() => (
              <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: 80 }}>
                <Ionicons name="search" size={64} color={COLORS.inactive} />
